...
에이전트가 자동으로 6단계(Phase 0~5)를 수행합니다. **v2.0.0부터 work-bridge로 격상** — knowledge-mind 저장소를 가진 사용자는 새 작업을 자신의 지식 그래프 배경에 자동으로 연결할 수 있습니다.

| Phase                          | 설명                                                              |
| ------------------------------ | ----------------------------------------------------------------- |
| 🔎 **0. 지식 탐색** *(선택)*    | knowledge-mind 저장소가 있으면 관련 노트·실패 사례 자동 스캔     |
| 📋 **1. 분석 & 계획**           | 아이디어 → 프로젝트명, 기술 스택, 핵심 기능 구조화                |
| 🧩 **2. 실행 가능성 + 자원 추천** | MVP 범위·난이도·리스크 + *(선택)* 재사용 가능한 KM 자원 매칭     |
| 📂 **3. GitHub 저장소 생성**    | `gh repo create`로 자동 생성                                      |
| 🏗️ **4. 프로젝트 스캐폴딩**     | 기술 스택에 맞는 파일/디렉토리 구조 생성                          |
| 📚 **4.5. KM 등록** *(선택)*    | knowledge-mind에 프로젝트 노트·portfolio·changelog 자동 등록      |
| 🚀 **5. 다음 스텝 + 환류 안내**  | 시간·난이도 포함 3~5개 액션 아이템 + carryback hook 안내          |

> **knowledge-mind 통합이란?**
> 본 에이전트의 작성자가 운영하는 개인 지식 그래프 시스템과의 연결입니다. `/Users/pheeree/PROJECT/knowledge-mind/` 경로에 knowledge-mind 저장소가 없으면 Phase 0·2-B·4.5는 건너뛰고 기본 5단계(1~5)만 작동합니다. 일반 사용자는 별도 설정 없이 그대로 사용 가능합니다.
